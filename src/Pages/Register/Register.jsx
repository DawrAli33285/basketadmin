...
function Register() {
  ...
  const registerNow = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post(`${BASE_URL}/adminRegister`, state);
      ...
      <div className="max-w-[1440px] mx-auto my-[60px] grid grid-cols-2 bg-[#FAFAFB]">
        ...
        <div className="w-full flex">
          <div className="m-auto">
            ...
            <h3 className="text-[16px]">Enter your email and password to sign up</h3>
            <div className="flex flex-col gap-[25px] mt-[20px]">
              ...
              <button onClick={registerNow} className="w-full mt-[20px] bg-[#F33] text-white py-[15px] rounded-[10px] font-bold">Register</button>
              <Link to='/'>
              <label className="cursor-pointer block text-[14px] mb-[8px]">Already have an account?</label>
              </Link>
            </div>
          </div>
        </div>
